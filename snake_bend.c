...
const char cmessage[]="hello";
__no_init char message[sizeof(cmessage];
...
void moveSnake();
char collisionDetection();
void eatApples();
//for walls
void clearWalls();
void createWall(char x1, char y1, char x2, char y2);
char searchWalls(char x, char y);
...
//wall data
__no_init char vwalls[10][3];
char hwalls[10][3];
char vindex;
char hindex;//index of the next free space in the array
...
void main(void)
{
     ...
     for (i=0;i<sizeof(cmessage);i++)
     message[i]=cmessage[i];
...
__no_init int bend_x [50] = {-1}; //Stores X Bend Coordinate Declare it big enough to accomodate maximum bends
int bend_y [50] = {-1};
...
while(bend_x[i]!=-1) //between every bend create a wall
{
 createWall(bend_x[i-1], bend_y[i-1], bend_x[i], bend_y[i]);
 halLcdLine(bend_x[i-1], bend_y[i-1], bend_x[i], bend_y[i], PIXEL_ON); 
 i++;
}

return searchWalls(head_x, head_y);
