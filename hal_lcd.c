 ...
 * @brief  Returns the LCD CGRAM value at location Address.
 * 
 * @param  x       The x-coor of the block to be read from the LCD. 
 * 
 * @param  y       The y-coor of the block to be read from the LCD. 
 *
 * @return Value   The value held at the specified address.
 *************************************************************************/
int halLcdReadPixel(int x, int y)
{
  unsigned int Address = (y << 5) + (x >> 3) ; 
  return halLcdReadBlock(Address);  
}
...
/**********************************************************************//**
 * @brief  Draw a Box of grayscale at coordinate (x,y) -> (x+size-1, y+size-1) to LCD 
 * 
 * @param  x         x-coordinate for grayscale value 
 * 
 * @param  y         y-coordinate for grayscale value
 ...
 * @return none
 *************************************************************************/
void halLcdPixelSize( int x, int y, unsigned char GrayScale, int size)
{
  for(int i = 0 ; i < size ; ++i)
  {
    for(int j = 0 ; j < size ; ++j)
    {
      halLcdPixel(x+i, y+j, GrayScale);
    }
  }
